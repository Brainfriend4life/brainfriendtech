...
function getServiceName(type: string) {
  switch (type) {
    ...
    case "NIN": return "NIN Verification";
    default: return type;
  }
}
  ...
  const filteredPurchases = useMemo(() => {
    ...
    return purchases.filter((purchase) => {
      ...
      const matchesSearch =
        ...
        purchase.provider?.toLowerCase().includes(query);
        ...
        <div className="rounded-2xl bg-gradient-to-r from-indigo-700 to-indigo-600 p-6 text-white shadow-lg sm:p-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="text-2xl font-bold sm:text-3xl">Purchases History</h1>
              <p className="mt-2 max-w-2xl text-sm text-indigo-100 sm:text-base">
                View and retrieve all your airtime, data, electricity, cable TV, exam PIN and NIN purchases in one place.
              </p>
            </div>
            <div className="rounded-xl bg-white/10 px-5 py-4 backdrop-blur-sm">
              <p className="text-xs text-indigo-100">Total Purchases</p>
          ...
          {loading ? (
            <div className="flex min-h-[350px] items-center justify-center">
              <div className="text-center">
                ...
                <p className="mt-3 text-sm font-medium text-gray-500">Loading your purchases...</p>
              </div>
            </div>
            ...
            <div className="flex min-h-[350px] flex-col items-center justify-center px-6 text-center">
              ...
              <h2 className="mt-5 text-lg font-bold text-gray-900">No purchases found</h2>
              <p className="mt-2 max-w-md text-sm text-gray-500">
                {search ? "No purchase matches your search." : "Your completed purchases will appear here."}
              </p>
            </div>
          ) : (
            <>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200 bg-gray-50 text-left">
                      ...
                      <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-gray-500">Provider</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredPurchases.map((purchase) => {
                      ...
                      return (
                        <tr key={`${purchase.type}-${purchase.id}`} className="transition hover:bg-gray-50">
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-semibold text-gray-900">{getServiceName(purchase.type)}</p>
                                <p className="max-w-[220px] truncate text-xs text-gray-500">{purchase.description}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4 text-sm font-medium text-gray-700">{purchase.provider || "—"}</td>
                          <td className="px-6 py-4 text-sm font-bold text-gray-900">{formatMoney(purchase.amount)}</td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${getStatusClass(purchase.status)}`}>
                              {purchase.status}
                            </span>
                          </td>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="font-bold text-gray-900">{getServiceName(purchase.type)}</p>
                              <p className="mt-0.5 truncate text-xs text-gray-500">{purchase.provider}</p>
                            </div>
                            <p className="shrink-0 font-bold text-gray-900">{formatMoney(purchase.amount)}</p>
                          </div>
                          <div className="mt-3 flex items-center justify-between">
                            <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${getStatusClass(purchase.status)}`}>
                              {purchase.status}
                            </span>
          ...
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="sticky top-0 flex items-center justify-between border-b border-gray-200 bg-white px-5 py-4">
              <div>
                ...
                <p className="text-xs text-gray-500">{getServiceName(selectedPurchase.type)}</p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-5 p-5">
              <div className="rounded-xl bg-indigo-50 p-4">
                <p className="text-xs font-medium text-indigo-600">Amount</p>
                <p className="mt-1 text-2xl font-bold text-indigo-900">{formatMoney(selectedPurchase.amount)}</p>
              </div>

              <div className="space-y-3">
                <DetailRow label="Service" value={getServiceName(selectedPurchase.type)} />
                <DetailRow label="Provider" value={selectedPurchase.provider || "—"} />
                <DetailRow label="Status" value={selectedPurchase.status} />
                <DetailRow
                  label="Reference"
                  value={selectedPurchase.reference}
                  copyable
                  onCopy={() => copyText(selectedPurchase.reference, "reference")}
                  copied={copied === "reference"}
                />
                <DetailRow label="Date" value={formatDate(selectedPurchase.createdAt)} />
                <DetailRow label="Description" value={selectedPurchase.description} />
              </div>

              {selectedPurchase.type === "EXAM_PIN" && selectedPurchase.details && (
                <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4">
                  <div className="mb-4 flex items-center gap-2">
                    <GraduationCap className="h-5 w-5 text-indigo-600" />
                    <h3 className="font-bold text-gray-900">Exam PIN Details</h3>
                  </div>
                  <div className="space-y-3">
                    <DetailRow
                      label="PIN"
                      value={selectedPurchase.details.pin}
                      copyable
                      onCopy={() => copyText(selectedPurchase.details.pin, "pin")}
                      copied={copied === "pin"}
                    />
                    <DetailRow
                      label="Serial Number"
                      value={selectedPurchase.details.serial}
                      copyable
                      onCopy={() => copyText(selectedPurchase.details.serial, "serial")}
              ...
              {selectedPurchase.type === "NIN" && selectedPurchase.details && (
                <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4">
                  <div className="mb-4 flex items-center gap-2">
                    <Fingerprint className="h-5 w-5 text-indigo-600" />
                    <h3 className="font-bold text-gray-900">NIN Verification Details</h3>
                  </div>
                  <div className="space-y-3">
                    <DetailRow
                      label="NIN"
                      value={selectedPurchase.details.nin}
                      copyable
                      onCopy={() => copyText(selectedPurchase.details.nin, "nin")}
                      copied={copied === "nin"}
                    />
                    <DetailRow label="Card Type" value={selectedPurchase.details.cardType} />
                    <DetailRow
                      label="Full Name"
                      value={
                        [selectedPurchase.details.firstName, selectedPurchase.details.middleName, selectedPurchase.details.surname]
                          .filter(Boolean)
                          .join(" ") || "—"
                      }
                    />
                    <DetailRow label="Gender" value={selectedPurchase.details.gender || "—"} />
                    <DetailRow label="Date of Birth" value={selectedPurchase.details.birthDate || "—"} />
                    <DetailRow label="Telephone" value={selectedPurchase.details.telephone || "—"} />
                    {selectedPurchase.details.transactionId && (
                      <DetailRow
                        label="Transaction ID"
                        value={selectedPurchase.details.transactionId}
                        copyable
                        onCopy={() => copyText(selectedPurchase.details.transactionId, "transactionId")}
                        copied={copied === "transactionId"}
                      />
                    )}
                  </div>

                  {selectedPurchase.details.photo && (
                    <div className="mt-5">
                      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-gray-500">Verification Photo</p>
                      <img
                        src={selectedPurchase.details.photo}
                  ...
                  {selectedPurchase.details.hasPdf && (
                    <button
                      type="button"
                      onClick={() => {
                        /**
                         * FIXED:
                         *
                         * This was previously missing the "/pdf"
                         * path segment, so it hit
                         * app/api/verification/nin/route.ts
                         * (POST-only) with a GET request and
                         * returned HTTP 405. It now correctly
                         * points at the PDF-retrieval route:
                         * app/api/verification/nin/[id]/pdf/route.ts
                         */
                        window.open(`/api/verification/nin/${selectedPurchase.id}/pdf`, "_blank");
                      }}
                      className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-indigo-700"
                    >
                      <Download className="h-4 w-4" />
                  ...
                  <button
                    ...
                    className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-3 text-sm font-bold text-indigo-700 transition hover:bg-indigo-50"
                  >
                    <Download className="h-4 w-4" />
  ...
  return (
    <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-3 last:border-0 last:pb-0">
      ...
      <div className="flex min-w-0 items-center gap-2 text-right">
        <span className="break-all text-sm font-semibold text-gray-900">{value || "—"}</span>
        {copyable && value && (
          <button
            type="button"
            onClick={onCopy}
